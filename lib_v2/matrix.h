...
public:
	...
	matrix<T>& operator*(const matrix<T> &other) const;
	matrix<T>& operator*(const T         &other) const;//scalar multiplication
	matrix<T>& operator/(const matrix<T> &other) const;
	matrix<T>& operator%(const matrix<T> &other) const;
	...
	const size_t& row_count() const { return data.size()/row_size; }
	...
	inline void fill(const args &...arguments){ data.assign<args>(max_size(), const args &...arguments); }//if it isnt trivially_default_constructible it wont do anything
